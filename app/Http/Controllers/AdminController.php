...
//             'experience' => 'nullable|string|max:255',
//             'aadhar_card_no' => 'nullable|string',
//             'teacher_image_name' => 'nullable|string', // Base64 string or URL
//         ], $messages);
...
//         // Handle image update only if a new Base64 image is provided
//         $newImageData = $request->input('teacher_image_name');

//         if ($newImageData === null || $newImageData === 'null') {
//             // Keep the existing image if the image is explicitly null
//             $validatedData['teacher_image_name'] = $staff->teacher_image_name;
//         } elseif ($newImageData && !str_contains($newImageData, '/storage/teacher_images/')) {
//             // Process a new base64 image if provided
//             if (preg_match('/^data:image\/(\w+);base64,/', $newImageData, $type)) {
//                 $newImageData = substr($newImageData, strpos($newImageData, ',') + 1);
//                 $type = strtolower($type[1]); // jpg, png, gif
...
//                 $validatedData['teacher_image_name'] = '/storage/teacher_images/' . $filename;
//             } else {
//                 throw new \Exception('Invalid image data');
//             }
//         } else {
//             // Keep existing image if no new image is provided or the image is an existing URL
//             $validatedData['teacher_image_name'] = $staff->teacher_image_name;
//         }
